...
function store_locally() {
    ...
    for (var i = 0; i < div_functions.length; i++) {
        function_id.push(div_functions[i].childNodes[1].id);
...
function Read_Inputs() {
    ...
    try {
        $("#Submit2").click(function() {
            ...
            $.ajax({
                type: "POST",
                url: "classes/store.php",
                data: {
                    input: in_data,
                    code: localStorage["orignal_code"],
                    output: eval(function_call + ";"),
...
function selected_fuction(e) {
    $("#includes").append("<div class='alert alert-success' style=' padding:5px; margin:5px; '><a href='#' class='close' data-dismiss='alert'>&times;</a><p id=" + e.options[e.selectedIndex].value + ">" + e.options[e.selectedIndex].childNodes[0].textContent + "</p></div>");
...
function ajaxFunction(str) {
    ...
    var url = "classes/ajax-search-demock.php";
    url = url + "?txt=" + str;
...
function ajax_tag_data(str) {
    ...
    var url = "classes/ajax-search-data.php";
    url = url + "?txt=" + str;
...
function selected_input(str) {
    ...
    var url = "classes/get_inputs.php";
